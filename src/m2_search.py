...
class BM25Search:
    ...
    def index(self, chunks: list[dict]) -> None:
        ...
        self.corpus_tokens = [
            segment_vietnamese(chunk["text"]).casefold().split()
    ...
    def search(self, query: str, top_k: int = BM25_TOP_K) -> list[SearchResult]:
        ...
        if self.bm25 is None or top_k <= 0:
            return []
        query_tokens = segment_vietnamese(query).casefold().split()
...
class DenseSearch:
    ...
    def _get_encoder(self):
        if self._encoder is None:
            ...
            try:
                ...
                print(
                    f"  ⚠️  {EMBEDDING_MODEL} is not cached; "
                    "using all-MiniLM-L6-v2 for offline development."
                )
                self._encoder = SentenceTransformer(
    ...
    def index(self, chunks: list[dict], collection: str = COLLECTION_NAME) -> None:
        ...
        dimension_getter = getattr(
            encoder, "get_embedding_dimension", encoder.get_sentence_embedding_dimension
        )
        vector_size = dimension_getter() or EMBEDDING_DIM
