...
def generate_hypothesis_questions(text: str, n_questions: int = 3) -> list[str]:
    ...
    if OPENAI_API_KEY:
        try:
            ...
            questions = [line.strip().lstrip("0123456789.-) ") for line in content.splitlines()]
...
def extract_metadata(text: str) -> dict:
    ...
    if OPENAI_API_KEY:
        try:
            ...
            response = OpenAI().chat.completions.create(
                model="gpt-4o-mini",
                messages=[
                    {"role": "system", "content": 'Trích xuất metadata và chỉ trả về JSON: {"topic": "...", "entities": ["..."], "category": "policy|hr|it|finance", "language": "vi|en"}'},
                    {"role": "user", "content": text},
                ],
                max_tokens=150,
            )
    ...
    if any(term in lowered for term in ("mật khẩu", "vpn", "bảo mật")):
        category = "it"
    elif any(term in lowered for term in ("lương", "chi phí", "tài chính")):
        category = "finance"
    elif any(term in lowered for term in ("nhân viên", "nghỉ", "thử việc")):
        category = "hr"
    else:
        category = "policy"
    ...
    if OPENAI_API_KEY:
        try:
            ...
            response = OpenAI().chat.completions.create(
                model="gpt-4o-mini",
                messages=[
                    {"role": "system", "content": """Phân tích đoạn văn và chỉ trả về JSON:
{
  ...
  "metadata": {"topic": "...", "entities": ["..."], "category": "policy|hr|it|finance", "language": "vi|en"}
    ...
    context = f"Trích từ {source}." if source else "Đoạn trích từ tài liệu nguồn."
    return {
